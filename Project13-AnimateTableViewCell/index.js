...
export const title = '13 - AnimateTableViewCell'
export const description = '列表动画'
...
export default class AnimateTableViewCell extends React.Component {
  ...
  componentWillMount () {
    ...
    tableData.map((item, index) => {
      animTransiton[index] = new Animated.Value(250)
      animOpacity[index] = new Animated.Value(0)
    })

    this.setState({animTransiton, animOpacity})
  }
  ...
  _renderRow (rowData, sectionID, rowID) {
    ...
    const translateY = this.state.animTransiton[rowID]
    const opacity = this.state.animOpacity[rowID]

    return (
      <Animated.View style={{opacity, transform: [{translateY}]}} key={rowID}>
